...
```json
{
  "tasks": {
    ...
    "compile": "deno compile --allow-read --allow-write --allow-net --allow-run=deno,python3,bash,sh,ruby,open,xdg-open,cmd --output rundoc src/main.ts"
  },
  "imports": {
    "hono": "npm:hono@4",
...
- Clicking "Run" executes the snippet and shows `hello from rundoc` in the output pre
- `/tmp/test.md` now contains an ` ```output ` block with `hello from rundoc`
...
Expected:
- Prints `rundoc running at http://localhost:7700`
...
Expected: `rundoc` binary created in the project root

- [ ] **Step 2: Test the compiled binary**

```bash
./rundoc README.md --no-open
```

Expected: `rundoc running at http://localhost:7700`
